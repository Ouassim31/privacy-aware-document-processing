...
function Process({
  ...
  return (
    <Card style={{ width: "20rem" }}>
      <Card.Body>
        <Card.Title className="row" data-cy="process-title">
          <p>{process._id}</p>{" "}
        ...
        <Card.Subtitle className="mb-2 text-muted">
          {view === "/landlord" ? process.applicant_id : process.landlord_id}
        </Card.Subtitle>
        <ListGroup variant="flush">
          <ListGroup.Item>
            <strong>State</strong> : {stateToText(process.process_state)}
          </ListGroup.Item>
          {view === "/landlord" && (
            <ListGroup.Item>
              ...
              <DescriptionCell
                updateDescription={(content) =>
                  updateDescription(process._id, content)
                }
      ...
      <Card.Footer className="d-flex gap-3">
        ...
        >
          {process.process_state === 3 && (
            <Dropdown.Item
              target="_blank"
              href={"https://explorer.iex.ec/bellecour/task/" + process.task_id}
            >
